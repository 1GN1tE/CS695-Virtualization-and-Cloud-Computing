...
void kvm_run_vm(struct vm *vm1, struct vm *vm2)
{
    ...
    while (1)
    {
        ...
        case KVM_EXIT_IO:
            ...
            {
                int port = current_vm->vcpus->kvm_run->io.port;
                if (port == 0x10 && current_vm == vm1)
                {
                    /* Produce VM1 write data */
                    values[i] = *(int *)((char *)(current_vm->vcpus->kvm_run) + current_vm->vcpus->kvm_run->io.data_offset);
                    printf("VMFD: %d Produced value: %d\n", current_vm->vm_fd, values[i]);

                    /* Schedule VM1 */
                    if(++i == 3)
                        current_vm = vm2;
                }
                else if (port == 0x11 && current_vm == vm2)
                {
                    /* Consumer VM2 read data */
                    *(int *)((char *)(current_vm->vcpus->kvm_run) + current_vm->vcpus->kvm_run->io.data_offset) = values[3 - i];
                    // printf("VMFD: %d Consumed the number\n", current_vm->vm_fd);
                }
                else if (port == 0x12 && current_vm == vm2)
                {
                    /* Consumer VM2 write data */
                    int value = *(int *)((char *)(current_vm->vcpus->kvm_run) + current_vm->vcpus->kvm_run->io.data_offset);
                    printf("VMFD: %d Consumed value: %d\n", current_vm->vm_fd, value);
