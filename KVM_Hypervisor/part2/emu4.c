...
int run_vm(struct vm *vm1, struct vm *vm2, struct vcpu *vcpu1, struct vcpu *vcpu2, size_t sz)
{
	...
	for (;;)
	{
		...
		case KVM_EXIT_IO:
			...
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0x15)
			{
				uint32_t buf_addr = *(uint32_t *)(p + vcpu->kvm_run->io.data_offset);
				if (vm == vm1)
				{
					buf1 = buf_addr;
				}
			...
			/* update buffer data in VMs and send prod_p */
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN && vcpu->kvm_run->io.port == 0x10)
			{
				uint32_t *arr;
				if (vm == vm1)
				{
					arr = (uint32_t *)(vm->mem + buf1);
				}
				else
				{
					arr = (uint32_t *)(vm->mem + buf2);
				}
				/* copying hypervisor buffer to vm buffer*/
				for (int i = 0; i < BUF_SIZE; i++)
				{
					arr[i] = buf[i];
				}

				*(uint32_t *)(p + vcpu->kvm_run->io.data_offset) = prod_p;
				// fprintf(stdout, "VMFD: %d Send prod_p: %d\n", vm->vm_fd, prod_p);
				// fflush(stdout);
				continue;
			}
			/* send cons_p */
			...
			{
				*(uint32_t *)(p + vcpu->kvm_run->io.data_offset) = cons_p;
				// fprintf(stdout, "VMFD: %d Send cons_p: %d\n", vm->vm_fd, cons_p);
				// fflush(stdout);
				continue;
			}
			/* get prod_p from producer and update local copy */
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0x13)
			{
				uint32_t old_p = prod_p;

				/* update part */
				prod_p = *(uint32_t *)(p + vcpu->kvm_run->io.data_offset);
				// fprintf(stdout, "VMFD: %d Recv prod_p: %d\n", vm->vm_fd, prod_p);
				// fflush(stdout);

				if (buf1 == 0)
				{
					fprintf(stderr, "buf1 not initialized");
					exit(1);
				}
				uint32_t *arr = (uint32_t *)(vm->mem + buf1);
				for (int i = 0; i < BUF_SIZE; i++)
				{
					buf[i] = arr[i];
				}
				...
				/* print part */
				uint32_t values = (prod_p - old_p + BUF_SIZE) % BUF_SIZE;

				printf("VMFD: %d Produced %d Values: [", vm->vm_fd, values);
				for (uint32_t i = 0; i < values; i++)
				{
					fprintf(stdout, "%d", buf[(old_p + 1 + i) % BUF_SIZE]);
					if (i < values - 1)
					{
						fprintf(stdout, " ");
					}
				...
				if (cons_p != prod_p)
				{
					uint32_t index = (cons_p + 1) % BUF_SIZE; 
					while (index != (prod_p + 1) % BUF_SIZE)
					{
						fprintf(stdout, "%d", arr[index]);

						index = (index + 1) % BUF_SIZE;

						if (index != (prod_p + 1) % BUF_SIZE)
						{
							fprintf(stdout, " ");
						}
					}
			...
			/* get cons_p from consumer and update local copy */
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0x14)
			{
				uint32_t old_c = cons_p;

				/* update part */
				cons_p = *(uint32_t *)(p + vcpu->kvm_run->io.data_offset);
				// fprintf(stdout, "VMFD: %d Recv cons_p: %d\n", vm->vm_fd, cons_p);
				// fflush(stdout);
				...
				uint32_t *arr = (uint32_t *)(vm->mem + buf2);

				/* print part */
				uint32_t values = (cons_p - old_c + BUF_SIZE) % BUF_SIZE;
				printf("VMFD: %d Consumed %d Values: [", vm->vm_fd, values);
				for (uint32_t i = 0; i < values; i++)
				{
					fprintf(stdout, "%d", buf[(old_c + 1 + i) % BUF_SIZE]);
					if (i < values - 1)
					{
						fprintf(stdout, " ");
					}
