	...
	for (;;)
	{
		...
		case KVM_EXIT_IO:
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0x10)
			{
				...
				uint32_t *temp = (uint32_t *)(vm->mem + buf_addr);

				fprintf(stdout, "VMFD: %d Produced Values:", vm->vm_fd);
				for (int i = 0; i < 5; i++)
				{
			...
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0x11)
			{
				/* VM2 to consume data */
				char *p = (char *)vcpu->kvm_run;
				uint32_t buf_addr = *(uint32_t*)(p + vcpu->kvm_run->io.data_offset);
			...
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0x12)
			{
				/* VM2 consumed data */
				char *p = (char *)vcpu->kvm_run;
				uint32_t buf_addr = *(uint32_t*)(p + vcpu->kvm_run->io.data_offset);
				...
				fprintf(stdout, "VMFD: %d Consumed Values:", vm->vm_fd);
				for (int i = 0; i < 5; i++)
				{
					fprintf(stdout, " %d", temp[i]);
				}
