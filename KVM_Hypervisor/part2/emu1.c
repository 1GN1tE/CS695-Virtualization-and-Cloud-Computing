...
void kvm_run_vm(struct vm *vm1, struct vm *vm2)
{
    ...
    while (1)
    {
        ...
        case KVM_EXIT_IO:
            ...
            // printf("VMFD: %d out port: %d, data: %d\n", current_vm->vm_fd, current_vm->vcpus->kvm_run->io.port, *(int *)((char *)(current_vm->vcpus->kvm_run) + current_vm->vcpus->kvm_run->io.data_offset));
            {
                ...
                if (port == 0x10 && current_vm == vm1)
                {
                    ...
                    value = *(int *)((char *)(current_vm->vcpus->kvm_run) + current_vm->vcpus->kvm_run->io.data_offset);
                    printf("VMFD: %d Produced value: %d\n", current_vm->vm_fd, value);
                }
                else if (port == 0x11 && current_vm == vm2)
                {
                    /* Consumer VM2 read data */
                    *(int *)((char *)(current_vm->vcpus->kvm_run) + current_vm->vcpus->kvm_run->io.data_offset) = value;
                    printf("VMFD: %d Consumed the number\n", current_vm->vm_fd);

                    /* VM2 has to be scheduled twice */
                    change = 0;
                }
                else if (port == 0x12 && current_vm == vm2)
                {
                    ...
                    /* Rescheduling alternative change */
                    change = 1;
                }
            }
            sleep(1);
        ...
        /* change current VM */
        if (change)
            current_vm = (current_vm == vm1) ? vm2 : vm1;
    }
