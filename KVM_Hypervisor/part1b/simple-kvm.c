...
int run_vm(struct vm *vm, struct vcpu *vcpu, size_t sz)
{
	...
	uint32_t exits = 0, exitOut = 0, exitIn = 0;
	...
	for (;;)
	{
		...
		case KVM_EXIT_IO:
			...
			{
				exitOut++;
				if (vcpu->kvm_run->io.port == 0xE9 && vcpu->kvm_run->io.size == 1)
				{
				...
				if (vcpu->kvm_run->io.port == 0xE9 && vcpu->kvm_run->io.size == 4)
				{
					fprintf(stdout, "%u\n", *(uint32_t *)(p + vcpu->kvm_run->io.data_offset));
					fflush(stdout);
					continue;
				}
				if (vcpu->kvm_run->io.port == 0xEB && vcpu->kvm_run->io.size == 4)
				{
					uint32_t str_addr = *(uint32_t *)(p + vcpu->kvm_run->io.data_offset);
					char *guest_str = (char *)(vm->mem + str_addr);
					fprintf(stdout, "%s", guest_str);
					fflush(stdout);
					continue;
				}
				if (vcpu->kvm_run->io.port == 0xE7 && vcpu->kvm_run->io.size == 4)
				{
					uint32_t buf_addr = *(uint32_t *)(p + vcpu->kvm_run->io.data_offset);
					char *buffer = (char *)(vm->mem + buf_addr);
					snprintf(buffer, 128, "IO in: %d\nIO out: %d\n", exitIn, exitOut);
					continue;
				}
				if (vcpu->kvm_run->io.port == 0xEC && vcpu->kvm_run->io.size == 4)
				{
					struct kvm_translation t;
					memset(&t, 0, sizeof(t));

					t.linear_address = *(uint32_t *)(p + vcpu->kvm_run->io.data_offset);

					if (ioctl(vcpu->vcpu_fd, KVM_TRANSLATE, &t) < 0)
					{
					...
					{
						hva = (uintptr_t)vm->mem + t.physical_address;
					}
					continue;
				}
			}
			...
			{
				exitIn++;
				if (vcpu->kvm_run->io.port == 0xEA)
				{
					*(uint32_t *)(p + vcpu->kvm_run->io.data_offset) = exits;
					continue;
				}
				if (vcpu->kvm_run->io.port == 0xEC)
				{
