...
# 1. Build image for the containers
...
./conductor.sh run cs-image cs-cont -- tail -f /dev/null > /dev/null &
./conductor.sh run es-image es-cont -- tail -f /dev/null > /dev/null &
...
IP_OUTPUT=$(./conductor.sh exec cs-cont -- ip -4 -brief address show dev cs-cont-inside)
C2_IP=`echo "$IP_OUTPUT" | grep cs-cont-inside | awk '{print $3}' | cut -d'/' -f1`
echo "IP_OUTPUT: $C2_IP"
...
./conductor.sh exec es-cont -- python3 /external-service/app.py "http://$C2_IP:8080"
