...
mount_overlay() {
    local lower_dirs="$1"
    local upper_dir="$2/diff"
    local work_dir="$2/work"
    merged_mount=$(realpath "$2/merged")

    local lower_opts=$(echo "$lower_dirs" | tr ':' ':')
    
    mkdir -p "$upper_dir" "$work_dir" "$merged_mount"
    
    # Mount with proper ordering
    mount -t overlay overlay \
        -o "lowerdir=$lower_opts,upperdir=$upper_dir,workdir=$work_dir" \
        "$merged_mount" || die "Overlay mount failed (lower: $lower_opts)"
}
...
handle_run() {
    ...
    # Subtask 3.f.1
    # Create new layer
    mkdir -p "$CACHEDIR/layers/$layer_hash/"{diff,work}

    # Subtask 3.f.2
    # Temporarily mount the overlay filesystem
    mount_overlay "$parent_layers" "$CACHEDIR/layers/$layer_hash"

    # Subtask 3.f.3
    # Execute the command in the new mount
    # mount -o bind /dev "$merged_mount/dev"
    unshare --uts --pid --mount --ipc --mount-proc --fork --kill-child \
        -R "$merged_mount" /bin/bash -c \
        "mount -t proc proc /proc && 
         ...
         chmod 755 / && $command && exit"
    
    # Subtask 3.f.4
    # Cleanup and record metadata
    umount "$merged_mount/proc" > /dev/null 2>&1 || :
    umount "$merged_mount/sys" > /dev/null 2>&1 || :
    umount "$merged_mount/dev/pts" > /dev/null 2>&1 || :
    umount "$merged_mount/dev" > /dev/null 2>&1 || :
    umount "$merged_mount/tmp" > /dev/null 2>&1 || :
    umount "$merged_mount"
...
handle_copy() {
    ...
    mkdir -p "$(dirname "$merged_mount/$dest")"
    cp -r "$src" "$merged_mount/$dest" || die "Failed to copy $src to $dest"
...
run() {
    ...
    local LAYER_STACK=$(< "$IMAGEDIR/$IMAGE/layers")
    
    # Convert to colon-separated absolute paths
    local LOWERDIRS=$(echo "$LAYER_STACK" | tr ':' '\n' | while read -r l; do
        realpath "$l" || die "Invalid layer path: $l"
    done | tr '\n' ':')
    LOWERDIRS="${LOWERDIRS%:}"  # Remove trailing colon

    # Verify directories exist
    echo "$LOWERDIRS" | tr ':' '\n' | while read -r dir; do
        [ -d "$dir" ] || die "Missing layer directory: $dir"
    done

    #Create container directories
    mkdir -p "$CONTAINERDIR/$NAME/"{upper,work,merged} || die "Failed to create container dirs"

    # Mount overlay
    mount -t overlay overlay \
        -o "lowerdir=$LOWERDIRS,upperdir=$(realpath "$CONTAINERDIR/$NAME/upper"),workdir=$(realpath "$CONTAINERDIR/$NAME/work")" \
        "$(realpath "$CONTAINERDIR/$NAME/merged")" || {
            echo "Failed layers: $LOWERDIRS" >&2
            die "Container mount failed (check dmesg | tail)"
        }
    # Subtask 3.d.2 - end
    ...
    # unshare --uts --pid --net --mount --ipc --mount-proc --fork --kill-child \
    # -R "$CONTAINERDIR/$NAME/rootfs" /bin/bash -c \
    # "mount -t sysfs sysfs /sys && chmod 755 / && $INIT_CMD_ARGS"

    # Subtask 3.d.3
    # Modify subtask 3.a.2 to use the overlay filesystem
    unshare --uts --pid --net --mount --ipc --mount-proc --fork --kill-child \
        -R "$CONTAINERDIR/$NAME/merged" /bin/bash -c \
        "mount -t proc proc /proc && 
         mount -t sysfs sysfs /sys &&
         mount -t tmpfs tmpfs /tmp &&
         mount -t devpts devpts /dev/pts &&
...
stop() {
    ...
    umount "$CONTAINERDIR/$NAME/merged/dev/pts" > /dev/null 2>&1 || :
    umount "$CONTAINERDIR/$NAME/merged/dev" > /dev/null 2>&1 || :
    umount "$CONTAINERDIR/$NAME/merged/tmp" > /dev/null 2>&1 || :
    
    # Subtask 3.d.4
    # Unmount the overlay filesystem
    umount "$CONTAINERDIR/$NAME/merged" 2>/dev/null || true
    
    # Deletes the container file
    rm -rf "$CONTAINERDIR/$NAME"
    [ -z "$(ls -1 "$CONTAINERDIR" 2>/dev/null || true)" ] && rm -f "$CACHEDIR/.HIGHEST_NUM" &&  iptables -P FORWARD DROP && iptables -F FORWARD && iptables -t nat -F
